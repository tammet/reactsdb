...
create table users ( -- all users are here: both admins and mobile users
  ...
  password varchar(50), -- not used for facebook or google case
...
create table sessions ( -- used only if/when login sessions needed
  id integer primary key default nextval('sessions_id_seq'),   
  sid varchar(200) not null unique, -- client-generated session id
  token varchar(200),
