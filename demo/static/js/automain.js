...
var AutoResource = React.createClass({
  displayName: 'AutoResource',
  ...
  simpleUpdateState: function(statechange) {
    //autoutils.debug("simpleUpdateState");
    //autoutils.debug(statechange);
    //autoutils.debug(this.state);
    /*
    // does statechange contain a version of data? If yes, go to top level.
    if (statechange.data && this.state.parent && this.state.parent.data && 
        statechange.data.uri==this.state.parent.data.uri &&
        statechange.viewdef && statechange.viewdef.name.indexOf("_version")>=0) {
      autoutils.debug("found version");
      statechange["parent"]=null;                    
    } 
    */    
  ...
  updateState: function(statechange)  { // {op:.., action: ... rowid:..., group:...,)    
    ...
    if (statechange.op=="messages") {
      console.log("messages in autoresource");
    ...
    } else if (statechange.op=="view" && statechange.action=="newversion") {  
      if (!isParentView) {        
        ...
        autoapi.createNewVersion(this,"view",this.state.viewdef,this.state.rowid,
               this.state.data["uri"],
               statechange.versionNr,this.state.parent); 
      } 
    } else if (statechange.op=="view" && statechange.action=="submitapproval") {  
      if (!isParentView) { // && _.has(this.state.datarow,"name")) {        
        autoutils.debug("approval");        
        autoutils.debug(this.state.data); 
        useViewdef=((statechange.viewdef) ? statechange.viewdef : this.state.viewdef);        
        autoapi.submitApproval(this,this.state.viewdef,this.state.data,this.state.parent);
  ...
  render: function() {    
    ...
    if (this.state.op=="overview") {
      return ce(autooverview.AutoOverview,{data:this.state.data});
      //if (!this.state.data) return ce("div","ha");
      // else return ce(autooverview.AutoOverview,{data:this.state.data});
    }
...
var AutoSimpleMenu = React.createClass({
  displayName: 'AutoSimpleMenu',
   handleButton: function(menuitem,e) {
    e.preventDefault();
...
var AutoLeftMenu = React.createClass({
  ...
  handleButton: function(name,viewmenu,restriction,component,menuselectname,e) {
    var usedata;
    e.preventDefault();
...
exports.AutoResource = AutoResource;
exports.AutoMain = AutoMain;
exports.getObjectLabel = getObjectLabel;
exports.AutoLeftMenu = AutoLeftMenu;
exports.AutoAlertMessage = AutoAlertMessage;

// ====== module end ==========

})(typeof exports === 'undefined'? this.automain = {} : exports);
